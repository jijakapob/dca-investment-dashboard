...
import { assetExamples, dummyMonthlyData } from "./data/dashboardSeed.js";
...
async function fetchAssetHistory(asset, start, end) {
  ...
  const response = await fetch(`${endpoint}?symbol=${encodeURIComponent(asset.symbol)}&start=${start}-01&end=${end}-31`);
...
function AssetForm({ onCalculate, loading }) {
  ...
  return h("form", { className: "rounded-2xl border border-line bg-white p-5 shadow-panel", onSubmit: submit }, [
    ...
    provider === "csv"
      ...
      : null,
    h("div", { key: "examples", className: "mt-4 flex flex-wrap gap-2" }, assetExamples.map((example) =>
      h("button", {
        key: example.code,
        type: "button",
        className: "rounded-full border border-line bg-soft px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-white",
        onClick: () => {
          setProvider(example.provider);
          setSymbol(example.code);
          setCurrency(example.currency);
          setName(example.code);
        },
